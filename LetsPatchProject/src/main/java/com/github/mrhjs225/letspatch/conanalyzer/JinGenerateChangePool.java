...
public class JinGenerateChangePool implements Serializable {
    ...
    public static void testgenerateChangePool(String path, String basicPath, String poolPath) {

        testSetPathEntries("collections", basicPath);
        testgenerateProjectChangePool(path, poolPath);

        testSetPathEntries("derby", basicPath);
        testgenerateProjectChangePool(path, poolPath);

        testSetPathEntries("groovy", basicPath);
        testgenerateProjectChangePool(path, poolPath);

        testSetPathEntries("hama", basicPath);
        testgenerateProjectChangePool(path, poolPath);

        testSetPathEntries("ivy", basicPath);
        testgenerateProjectChangePool(path, poolPath);

        testSetPathEntries("lucene", basicPath);
        testgenerateProjectChangePool(path, poolPath);

        testSetPathEntries("mahout", basicPath);
        testgenerateProjectChangePool(path, poolPath);

        testSetPathEntries("hadoop", basicPath);
        testgenerateProjectChangePool(path, poolPath);
    }

    // to test generate changePool by project
    public static void testgenerateProjectChangePool(String path, String poolPath) {
        ChangePoolGenerator cpg = new ChangePoolGenerator(new TestContextIdentifier());
        cpg.pool.setPoolDir(new File(poolPath));
        String[] classPathEntries = new String[] { classPathString };
        String[] sourcePathEntries = new String[] { sourcePathString };
        File beforePatchFile;
        File afterPatchFile;
        ...
        cpg.pool.storeTo(new File(poolPath), true);
        pool = cpg.pool;
        System.out.println(targetProjectName + " done");
    }

    public static void testSetPathEntries(String targetProject, String basicPath) {
        JinGenerateChangePool.targetProjectName = targetProject;
        ...
        if (targetProject.equals("collections")) {
            ...
            classPathString = basicPath + "hadoop-common" + "/src";
            sourcePathString = basicPath + "hadoop-common"; // here
        } else if (targetProject.equals("hama")) {
            classPathString = basicPath + targetProject + "/core/target";
            sourcePathString = basicPath + targetProject + "/src";
        } else if (targetProject.equals("ivy")) {
            ...
            classPathString = basicPath + "lucene-solr" + "/bin";
            sourcePathString = basicPath + "lucene-solr" + "/src";
        } else if (targetProject.equals("mahout")) {
            classPathString = basicPath + targetProject + "/core/target";
            sourcePathString = basicPath + targetProject + "/src";
        } else if (targetProject.equals("pdfbox")) {
